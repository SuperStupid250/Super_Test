...
nodelist = ['00124b0023acf55f']
payload={}                                                                                                              #json在python相当于字典 创建个字典储存payload


for p in nodelist:                                                                                                      #用i分别充当nodelist里面的每一项
                                                                                                                        #设置好接口url
  url = "https://api.sync-sign.com/v2/user/monitor?type=online&deviceType=NODE&nodeId={}".format(p)
                                                                                                                        #设置好请求头  ：里面带token访问
  headers = {
  ...
  f = csv.reader(open('C:/Users/Administrator/Downloads/{}.csv'.format(p), 'r')) #打开csv，文件名为当前循环到的nodeid
  num = 0                                                                                                               #初始化false出现的次数
  unlinetime = []
  ...
  for i in f:                                                                                                           #用i 遍历csv读出来的每一行
    if i[3][0:2] == '22':                                                                                               #如果年份=22 进入下一层判断
      if i[3][3:5] == '01':                                                                                             #如果月份=11  进入下一层判断
        if i[3][6:8] >= '17':                                                                                           #如果日期>=18  进入下一层判断
          if i[4] == 'false':                                                                                           #如果在线状态为false 执行一下语句
            unlinetime.append(i[3])
            num += 1                                                                                                    #num自加1
          else:                                                                                                         #否则
            continue                                                                                                    #提前结束此次循环 进入下次循环(下一个nodeid)

  print(str(p)+'掉线次数： '+str(num)+' /时间： '+str(unlinetime))                                                       #打印 nodeid + false出现的次数
  f = open('C:/Users/Administrator/Desktop/0120.csv', mode='a+', encoding='utf-8') #打开csv
  csv_writer = csv.writer(f)                                                                                            #进入写模式
  csv_writer.writerow([str(p[1]), num])
                                                                                                                      #把本次结果写进去
  os.remove('C:/Users/Administrator/Downloads/{}.csv'.format(p))
